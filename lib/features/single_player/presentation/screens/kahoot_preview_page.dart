...
class _KahootPreviewScreenState extends State<KahootPreviewScreen> {
              ...
              child: Column(
                ...
                children: [
                  ...
                  if ((preview.description ?? '').isNotEmpty)
                    Text(preview.description!, style: Theme.of(context).textTheme.bodyMedium),
                  const SizedBox(height: 16),

                  // Progress card (if in progress or completed)
                  if (preview.isInProgress || preview.isCompleted) _buildProgressCard(preview),
      ...
      children: [
        ClipRRect(
          ...
          child: thumb != null && thumb.isNotEmpty
              ? Image.network(thumb, height: 180, width: double.infinity, fit: BoxFit.cover, errorBuilder: (_, __, ___) {
                  return Container(height: 180, color: Colors.grey.shade200, child: const Center(child: Icon(Icons.image_not_supported)));
                })
              : Container(height: 180, color: Colors.grey.shade200, child: const Center(child: Icon(Icons.image))),
        ),
        Positioned(
          right: 8,
          top: 8,
          child: CircleAvatar(
            ...
            child: Icon(preview.isFavorite ? Icons.favorite : Icons.favorite_border, color: preview.isFavorite ? Colors.red : Colors.black54),
          ),
        ),
      ],
    );
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Estado del intento', style: TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          Row(children: [
            Chip(label: Text(preview.isInProgress ? 'En progreso' : 'No iniciado')),
            const SizedBox(width: 8),
            if (preview.isCompleted) Chip(label: Text('Completado'), backgroundColor: Colors.green.shade100),
          ]),
          const SizedBox(height: 8),
          if (gs != null) ...[
  ...
  Widget _buildActionButtons(GameController controller, preview) {
    ...
    return Column(
      ...
      children: [
        ...
        if (inProgress)
          ElevatedButton(
            onPressed: controller.isLoading || controller.isSubmitting ? null : () => _resumeAttempt(context, controller, preview),
        ...
        if (inProgress)
          TextButton(
            onPressed: controller.isLoading || controller.isSubmitting ? null : () => _confirmRestart(context, controller),
        ...
        if (completed)
          OutlinedButton(
            onPressed: controller.isLoading ? null : () => _viewSummary(context, controller, preview),
  ...
  Future<void> _startNewAttempt(BuildContext context, GameController controller) async {
    ...
    if (attempt != null) {
      ...
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => GamePage(attemptId: attempt.attemptId)));
  ...
  Future<void> _resumeAttempt(BuildContext context, GameController controller, dynamic preview) async {
    ...
    if (attemptId != null && attemptId.isNotEmpty) {
      ...
      if (controller.lastFailure != null) _showSnack(controller.lastFailure!.message);
      return;
    }
  ...
  Future<void> _viewSummary(BuildContext context, GameController controller, dynamic preview) async {
    final attemptId = preview.gameState?.attemptId ?? (await GameStorage.getAttempt())['attemptId'];
    if (attemptId == null) {
  ...
  Future<void> _confirmRestart(BuildContext context, GameController controller) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.pop(context, true), child: const Text('Reiniciar')),
        ],
      ),
    );
