...
class MultiplayerBloc extends Bloc<MultiplayerEvent, MultiplayerState> {
  ...
  MultiplayerBloc({
    ...
    on<_OnRoomJoinedUpdate>((event, emit) =>
        emit(state.copyWith(status: MultiplayerStatus.inLobby)));
    on<_OnLobbyUpdate>((event, emit) => emit(
        state.copyWith(lobby: event.lobby, status: MultiplayerStatus.inLobby)));
    ...
    on<_OnAnswerCountUpdate>(
        (event, emit) => emit(state.copyWith(answersReceived: event.count)));
    on<_OnSocketErrorReceived>((event, emit) => emit(state.copyWith(
        status: MultiplayerStatus.error, failure: event.failure)));
  ...
  Future<void> _onCreateSessionStarted(
    ...
    final result =
        await _createSession(kahootId: event.kahootId, jwt: event.jwt);
    ...
    result.fold(
      ...
      (session) {
        ...
        add(OnConnectStarted(
            role: ClientRole.host, pin: session.sessionPin, jwt: event.jwt));
      },
    );
  }
  ...
  void _initSubscriptions() {
    ...
    _playerSuccessSub =
        _listenPlayerSuccess().listen((_) => add(_OnPlayerConnectedSuccess()));
    _roomJoinedSub =
